  ...
  return createPortal(
    ...
    >
      <div 
        ...
        className="relative bg-background border border-hud flex flex-col w-full max-w-3xl"
      >
        ...
        <div ref={contentRef} className="flex-1 flex flex-col p-8 md:p-12 overflow-y-auto">
          ...
          <div className="flex justify-between items-start mb-12">
            ...
            <button 
              onClick={handleClose}
              className="bracket-btn font-mono text-[13px] md:text-[15px] tracking-widest uppercase"
            >
          ...
          <div className="flex-1">
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black font-sans uppercase tracking-tighter mb-8 text-primary leading-none">
              {displayText}
            </h2>
            <p className="text-muted font-mono text-base mb-12 leading-relaxed max-w-2xl">
              {service.description}
            </p>

            {service.benefits && service.benefits.length > 0 && (
              <div className="mb-12">
                <h3 className="font-mono text-xs text-muted mb-6 tracking-widest uppercase">
                  [KEY_CAPABILITIES]
                </h3>
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {service.benefits.map((benefit, i) => (
                    <li key={i} className="flex items-start gap-4 text-sm font-mono text-muted">
                      <span className="text-muted mt-0.5">{'>'}</span> {benefit}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {service.technologies && service.technologies.length > 0 && (
            <div className="pt-8 border-t border-hud mt-8">
              <h3 className="font-mono text-xs text-muted mb-6 tracking-widest uppercase">
                [TECH_STACK]
              </h3>
              <div className="flex flex-wrap gap-2">
                {service.technologies.map((tech, i) => (
                  <span key={i} className="font-mono text-xs text-muted uppercase border border-hud px-3 py-1.5 tracking-widest bg-primary/5 whitespace-nowrap">
                    {tech}
                  </span>
                ))}
              </div>
